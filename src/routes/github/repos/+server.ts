...
import type { TypeReposResponse } from '$lib/types/services/github-repos.response.types.js'
...
export const GET = async (params) => {
	...
	if (!isValid) {
		...
		const response = await getGithubUserData(username)

		return new Response(toString(response), GetMethodServer)
	}
}
...
async function getGithubUserData(username: string | null) {
	const URL = `${PRIVATE_GITHUB_API_USERS}/${username}/repos`
	const { data } = await axios.get(URL)
	return data as TypeReposResponse[]
}
